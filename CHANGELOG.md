...
## 2026-09-01 — 全語に例文 + 誤り訂正

- example のなかった 176 語すべてに例文を追加（全 200 語で example あり）
- **暁通 → 通暁（つうぎょう）に訂正**：「暁通」は辞書にない語。同義の標準表記に直した
- `tests/idioms.test.ts` に不変条件を追加：全語に例文がある／読みはひらがなのみ／語の重複なし
- `npm test` 52 件緑・静的エクスポートビルド成功
