...
## 2026-08-31 — 4択の設計をやり直し（正解が複数ある問題を一掃）

全200問を再点検した結果、**誤答を「正解の類義語」にしていたため、150問以上で正解が複数成立していた**（例: 斡旋の選択肢が 仲介／周旋／口利き）。誤答の作り方を根本から変更。

- **新方針**: 誤答は「同じ意味ドメイン(`group`) かつ 同じ字数 かつ 別の意味クラスタ(`cluster`)」の**語彙内の語**から選ぶ。分野が同じなので紛らわしく、意味が別なので正解は一つに定まる
- `Idiom.cluster` を追加。同義・類義の語（顧慮/酌量/斟酌/忖度、直言/吐露/披瀝、罷免/更迭、首尾一貫/徹頭徹尾/理路整然 など計51語）を24クラスタにまとめ、互いに誤答へ出さない
- 手作りの類義語 `distractors` 600語は廃止。同字数の候補が語彙内にない3語（現・押し並べて・揺蕩う）だけ残す
- 語釈は語彙データの `meaning` をそのまま使うため、誤答の語釈も出典どおりで正確になった
- **回帰テスト追加**: 全200語について「4択が作れる」「誤答に同クラスタの語が混ざらない」「重複語がない」「字数が揃う」を検証
