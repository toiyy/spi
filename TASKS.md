...
- [ ] **実機で触って確認** - `npm run dev` → 出題〜結果〜履歴の一連を確認（人間の目視）
- [ ] **意味クラスタの継続チェック** - 同義なのに別クラスタのままの組があれば追加する（見つけ次第。テストが機械的に検出する仕組みあり）
...
- [x] ~~4択の設計をやり直し~~ (2026-08-31) - 誤答が類義語で正解が複数成立していた問題を根本修正。`cluster`(同義グループ24件/51語)を導入し、誤答は「同ドメイン・同字数・別クラスタ」の語彙内語から自動選定。手作り誤答600語は廃止。全200問の回帰テスト付き
- [x] ~~全語に例文を追加~~ (2026-09-01) - example のなかった176語に例文を付与。暁通→通暁の誤りも訂正。読み・重複のテスト追加
